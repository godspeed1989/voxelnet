...
__device__ inline float  cal_z_intersect(float cz1, float h1, float cz2, float h2) {
  ...
  if (b1z1 > b2z2 || b2z1 > b1z2) {
    return 0;
  }
  else if (b2z1 <= b1z1 && b1z1 <= b2z2) {
    if (b1z2 <= b2z2)
      return h1;
    else
      return b2z2 - b1z1;
  }
  else if (b1z1 < b2z1 && b2z1 < b1z2) {
    if (b2z2 <= b1z2)
      return h2;
    else
      return b1z2 - b2z1;
...
__device__ inline float devRotateIoU_3d(float const * const region1, float const * const region2) {
  ...
  if((fabs(region1[0] - region2[0]) < 1e-5) &&
     ...
     (fabs(region1[3] - region2[3]) < 1e-5) &&
     (fabs(region1[4] - region2[4]) < 1e-5) &&
     (fabs(region1[5] - region2[5]) < 1e-5) &&
     (fabs(region1[6] - region2[6]) < 1e-5)) {
    return 1.0;
  }

  float xy_region1[5];
  xy_region1[0] = region1[0];
  xy_region1[1] = region1[1];
  xy_region1[2] = region1[4];
  xy_region1[3] = region1[5];
  xy_region1[4] = region1[6];
  float xy_region2[5];
  xy_region2[0] = region2[0];
  xy_region2[1] = region2[1];
  xy_region2[2] = region2[4];
  xy_region2[3] = region2[5];
  xy_region2[4] = region2[6];

  float xy_area1 = xy_region1[2] * xy_region1[3];
  float xy_area2 = xy_region2[2] * xy_region2[3];
  float xy_area_inter = inter(xy_region1, xy_region2);
...
__global__ void overlaps_kernel_3d(const int N, const int K, const float* dev_boxes,
                                   const float * dev_query_boxes, float* dev_overlaps) {
  ...
  __shared__ float block_query_boxes[threadsPerBlock * 7];
  if (threadIdx.x < col_size) {
    block_query_boxes[threadIdx.x * 7 + 0] =
        dev_query_boxes[(threadsPerBlock * col_start + threadIdx.x) * 7 + 0];
    block_query_boxes[threadIdx.x * 7 + 1] =
        dev_query_boxes[(threadsPerBlock * col_start + threadIdx.x) * 7 + 1];
    block_query_boxes[threadIdx.x * 7 + 2] =
        dev_query_boxes[(threadsPerBlock * col_start + threadIdx.x) * 7 + 2];
    block_query_boxes[threadIdx.x * 7 + 3] =
        dev_query_boxes[(threadsPerBlock * col_start + threadIdx.x) * 7 + 3];
    block_query_boxes[threadIdx.x * 7 + 4] =
        dev_query_boxes[(threadsPerBlock * col_start + threadIdx.x) * 7 + 4];
    block_query_boxes[threadIdx.x * 7 + 5] =
        dev_query_boxes[(threadsPerBlock * col_start + threadIdx.x) * 7 + 5];
    block_query_boxes[threadIdx.x * 7 + 6] =
        dev_query_boxes[(threadsPerBlock * col_start + threadIdx.x) * 7 + 6];
  }

  if (threadIdx.x < row_size) {
    block_boxes[threadIdx.x * 7 + 0] =
        dev_boxes[(threadsPerBlock * row_start + threadIdx.x) * 7 + 0];
    block_boxes[threadIdx.x * 7 + 1] =
        dev_boxes[(threadsPerBlock * row_start + threadIdx.x) * 7 + 1];
    block_boxes[threadIdx.x * 7 + 2] =
        dev_boxes[(threadsPerBlock * row_start + threadIdx.x) * 7 + 2];
    block_boxes[threadIdx.x * 7 + 3] =
        dev_boxes[(threadsPerBlock * row_start + threadIdx.x) * 7 + 3];
    block_boxes[threadIdx.x * 7 + 4] =
        dev_boxes[(threadsPerBlock * row_start + threadIdx.x) * 7 + 4];
    block_boxes[threadIdx.x * 7 + 5] =
        dev_boxes[(threadsPerBlock * row_start + threadIdx.x) * 7 + 5];
    block_boxes[threadIdx.x * 7 + 6] =
        dev_boxes[(threadsPerBlock * row_start + threadIdx.x) * 7 + 6];
  }
  ...
  if (threadIdx.x < row_size) {
    for(int i = 0;i < col_size; i++) {
      int offset = row_start*threadsPerBlock * K + col_start*threadsPerBlock + threadIdx.x*K+ i ;
      dev_overlaps[offset] = devRotateIoU_3d(block_boxes + threadIdx.x * 7, block_query_boxes + i * 7);
    }
  }
}
...
void _overlaps_3d(float* overlaps,const float* boxes,const float* query_boxes, int n, int k, int device_id) {
  ...
  overlaps_kernel_3d<<<blocks, threads>>>(n, k,
                                          boxes_dev,
                                          query_boxes_dev,
                                          overlaps_dev);
...
__global__ void rotate_nms_kernel_3d(const int n_boxes, const float nms_overlap_thresh,
  ...
  if (threadIdx.x < row_size) {
    ...
    int i = 0;
    unsigned long long t = 0;   // threadsPerBlock = sizeof(unsigned long long) * 8
    int start = 0;
    if (row_start == col_start) {
      start = threadIdx.x + 1;
    }
    for (i = start; i < col_size; i++) {
      if (devRotateIoU_3d(cur_box, block_boxes + i * 8) > nms_overlap_thresh) {
        t |= 1ULL << i;
      }
    }
...
void _rotate_nms_3d(int* keep_out, int* num_out, const float* boxes_host, int boxes_num,
  ...
  dim3 threads(threadsPerBlock);
  rotate_nms_kernel_3d<<<blocks, threads>>>(boxes_num,
                                            nms_overlap_thresh,
                                            boxes_dev,
                                            mask_dev);
