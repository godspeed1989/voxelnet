...
def train():
    ...
    predict_summary_list.append(tf.summary.scalar('loss_pred', loss_pred))
    if False:
        tvars = tf.trainable_variables()
        lossL1 = tf.add_n([ tf.reduce_sum(tf.abs(v)) for v in tvars if 'bias' not in v.name]) * 0.001
        predict_summary_list.append(tf.summary.scalar('lossL1', lossL1))
        loss = loss_pred + lossL1
    else:
        loss = loss_pred
    predict_summary = tf.summary.merge(predict_summary_list)
    ...
    for epoch in range(2):
        for f in f_lidar:
            ...
            if point_cloud is None:
                continue
            step = step + 1
            loss_val, _, pred_summary_val = sess.run([loss, train, predict_summary],
